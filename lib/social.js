...
function SocialTracker(tracker, opts) {
  ...
  delegate(document, '[' + this.opts.prefix + '-network]', 'click',
...
SocialTracker.prototype.handleEventClicks = function(event) {
  ...
  this.tracker.send('social', {
    socialNetwork: link.getAttribute(this.opts.prefix + '-network'),
    socialAction: link.getAttribute(this.opts.prefix + '-action'),
    socialTarget: link.getAttribute(this.opts.prefix + '-target')
  });
};
