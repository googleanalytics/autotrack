...
/**
 * Returns a unique number for this page.
 * @return {number}
 */
export function uid() {
  return ++uid_;
...
/**
 * The native `requestIdleCallback()` function or `setTimeout()` if the
 * browser doesn't support it.
 * @param {!Function} callback
 * @return {number}
 */
export const rIC = (callback) => supportsRequestIdleCallback_ ?
    requestIdleCallback(callback) : setTimeout(callback, 0);


/**
 * The native `cancelIdleCallback()` function or `clearTimeout()` if the
 * browser doesn't support it.
 * @param {number} handle
 * @return {undefined}
 */
export const cIC = (handle) => supportsRequestIdleCallback_ ?
