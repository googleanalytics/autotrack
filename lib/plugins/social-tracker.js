...
function SocialTracker(tracker, opts) {
  ...
  this.opts = defaults(opts, {
    attributePrefix: 'data-social'
  });

  this.tracker = tracker;

  var prefix = this.opts.attributePrefix;
  var selector = '[' + prefix + 'social-category][' + prefix + 'social-action]';
 ...
 */
SocialTracker.prototype.handleEventClicks = function(event) {

  var link = event.delegateTarget;
  var prefix = this.opts.attributePrefix;

  this.tracker.send('social', {
    socialNetwork: link.getAttribute(prefix + '-network'),
    socialAction: link.getAttribute(prefix + '-action'),
    socialTarget: link.getAttribute(prefix + '-target')
  });
};
