...
OutboundLinkTracker.prototype.handleLinkInteractions = function(event, link) {
  ...
  if (this.opts.shouldTrackOutboundLink(link, parseUrl)) {
    ...
    // ctrl/shift/meta modifiers behave the same as _blank
    if (event.ctrlKey || event.shiftKey || event.metaKey || event.which == 2) {
      target = '_blank';
    }

    // Stops the link navigation and waits until the hit is complete
    // (with timeout) for browsers that don't support beacon.
