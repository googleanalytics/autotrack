...
var DEFAULT_SESSION_TIMEOUT = 30; // In minutes.
...
PageVisibilityTracker.prototype.scheduleNextHeartbeat = function() {
  ...
  this.heartbeatTimeout = setTimeout(function() {
    ...
    if (visibilityState == VISIBLE &&
      ...
      this.sendPageVisibilityEvent('heartbeat', VISIBLE);
      this.scheduleNextHeartbeat();
...
PageVisibilityTracker.prototype.overrideTrackerSendMethod = function() {
  ...
  this.tracker.send = function() {
    ...
    // clear the stored visibility change time and reschedule heartbeats.
    if (isSessionExpired) {
      this.store.set({time: null});

      // If the current window/tab is visible, ensure it's set as the stored
      // active page ID and schedule a heartbeat.
      if (this.getVisibilityState() == VISIBLE) {
        this.store.set({
          state: VISIBLE,
          pageId: PAGE_ID,
