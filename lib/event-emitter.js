...
export default class EventEmitter {
   ...
   * @param {Function} handler The handler to be invoked when the passed
   *     event is emitted.
   */
  on(event, handler) {
    this.getRegistry_(event).push(handler);
  }
  ...
  off(event, handler) {
    if (event && handler) {
      ...
      const handlerIndex = eventRegistry.indexOf(handler);
      if (handlerIndex > -1) {
   ...
   * @param {string} event The event name.
   * @param {Array} args The arguments to be passed to the handler.
   */
  emit(event, ...args) {
    this.getRegistry_(event).forEach((handler) => handler(...args));
  }
   ...
   * @private
   * @param {string} event The event name.
   * @returns {Array} An array of handler functions.
   */
  getRegistry_(event) {
