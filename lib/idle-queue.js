...
export default class IdleQueue {
  ...
  constructor(tracker) {
    ...
    if (isSafari()) {
      addEventListener('beforeunload', this.runCallbacksImmediately_, true);
    }
  }
  ...
  destroy() {
    ...
    // prevent the page from being eligible for bfcache.
    if (isSafari()) {
      removeEventListener('beforeunload', this.runCallbacksImmediately_, true);
    }
  }

  /**
   * Runs all added callbacks as microtasks.
   */
  runCallbacksImmediately_() {
    if (this.pendingIdleCallbacks > 0) {
      Object.keys(this.idleCallbacks_).forEach((id) => {
        ...
        // console.log('running as microtask', id, callback.toString());
