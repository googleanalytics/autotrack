...
export default class Store extends EventEmitter {
  ...
  static getOrCreate(trackingId, namespace, opts = {}) {
    ...
    if (!instances[key]) {
      instances[key] = new Store(key, opts);
   ...
   * @param {{bypassCache: (boolean)}=} param1
   *     bypassCache: If true, cached data will be ignored.
   * @return {!Object} The stored data merged with the defaults.
   */
  get({bypassCache = false} = {}) {
    if (this.cache_ && !bypassCache) {
      return this.cache_;
    }
  ...
  set(newData) {
    ...
    // isn't newer than the data we're about to set.
    ...
    if (timestampKey && typeof newData[timestampKey] === 'number') {
      oldData = this.get({bypassCache: true});
    ...
    this.cache_ = assign(oldData, newData);
    if (Store.isSupported_()) {
      try {
