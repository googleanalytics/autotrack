 ...
 */
export function addHookBefore(obj, methodName, fn) {
  getHook(obj, methodName).on(BEFORE, fn);
}
 ...
 * @param {!Function} fn The callback to add.
 */
export function addHookAfter(obj, methodName, fn) {
  getHook(obj, methodName).on(AFTER, fn);
}


/**
 * Removes a callback added via `addHookBefore()`. If the callback is the
 ...
 */
export function removeHookBefore(obj, methodName, fn) {
  getHook(obj, methodName).off(BEFORE, fn)
}


/**
 * Removes a callback added via `addHookAfter()`. If the callback is the
 * only callback added, the method is unwrapped and restored.
 * @param {!Object} obj The object containing the method to remove a callback.
 * @param {string} methodName The name of the method on the object.
 * @param {!Function} fn The callback to remove.
 */
export function removeHookAfter(obj, methodName, fn) {
  getHook(obj, methodName).off(AFTER, fn)
}
 ...
 * @return {!Hook}
 */
function getHook(obj, methodName) {
  let hook = hooks
