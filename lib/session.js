...
function initSessionControl(tracker) {
  ...
  var newSendHitTask = function(model) {
    ...
    if (isInteractionHit) {
      storage.set(trackingId, {hitTime: +new Date()});
    }
  ...
  trackers[trackingId] = {
    oldSendHitTask: oldSendHitTask,
    newSendHitTask: newSendHitTask,
  };
}
 ...
 */
function hasExpired(tracker, sessionExpiryTime, timeZone) {
  var now = new Date();
  var trackingId = tracker.get('trackingId');
  var oldHitTime = storage.get(trackingId, 'hitTime');
