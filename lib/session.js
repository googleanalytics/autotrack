...
export default class Session {
  ...
  constructor(tracker, timeout, timeZone) {
    ...
    // This will be set lazily since it's expensive to create.
  ...
  get store() {
    if (!this.store_) {
      ...
      this.store_ = Store.getOrCreate(
          this.tracker.get('trackingId'), 'session', {
        defaults: defaultProps,
        timestampKey: 'hitTime',
      });

      // Ensure the session has an ID.
      if (!this.store_.get().id) {
        this.store_.set(/** @type {SessionStoreData} */ ({id: uuid()}));
      }
    }
    return this.store_;
  }
  ...
  datesAreDifferentInTimezone(d1, d2) {
    const dateTimeFormatter = getOrCreateDateTimeFormatter(this.timeZone);

    if (dateTimeFormatter) {
      return dateTimeFormatter.format(d1) != dateTimeFormatter.format(d2);
    } else {
      return false;
    }
  }
...
const dateTimeFormatterCache_ = {};
const getOrCreateDateTimeFormatter = (timeZone) => {
  if (!(timeZone in dateTimeFormatterCache_)) {
    // Some browser doesn't support various features of the
    // `Intl.DateTimeFormat` API, so we have to try/catch it.
    try {
      dateTimeFormatterCache_[timeZone] =
          new Intl.DateTimeFormat('en-US', {timeZone});
    } catch (err) {
      dateTimeFormatterCache_[timeZone] = null;
    }
  }
  return dateTimeFormatterCache_[timeZone];
};
