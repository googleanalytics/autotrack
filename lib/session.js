...
export default class Session {
  ...
  constructor(tracker, timeout, timeZone) {
    ...
    this.hasSentHit_ = false;

    // Binds methods.
    this.sendHitTaskHook = this.sendHitTaskHook.bind(this);

    // Hooks into the trackers sendHitTask method.
    Hook.addAfter(tracker, 'sendHitTask', this.sendHitTaskHook);
  ...
  /**
   * Returns true if the tracker has a record of an hit occuring as well as a
   * record of it being from a previous session. If any of that information is
   * missing, err on the side of caution.
   ...
   */
  isLastTrackerHitFromPreviousSession() {
    const globalSessionCount = this.store.get().sessionCount;
    const trackerSessionCount = this.sessionCount_;
    return this.hasSentHit_ && globalSessionCount > trackerSessionCount;
  }
   ...
   */
  destroy() {
    Hook.removeAfter(this.tracker, 'sendHitTask', this.sendHitTaskHook);
    this.store.destroy();
